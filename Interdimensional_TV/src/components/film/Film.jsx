...
const MovieRow = ({ title, category }) => {
  ...
  // Ensure buttons are updated when items are rendered/changed
  useEffect(() => {
    setTimeout(updateButtons, 50);
  }, [items]);
...
>>>>>>> GiraudoAndrea_modificheMain
              ...
              {/* Preferiti button (same behavior as TitleCard) */}
              <button
                className="fav-circle"
                onClick={() => (fav ? removeFavourite(it.id) : addFavourite({ id: it.id, media_type: 'movie', poster_path: it.backdrop_path || null, title: it.title || null }))}
              >
