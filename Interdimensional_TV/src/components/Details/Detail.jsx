...
export default function Detail({ mediaType }) {
  ...
  // =========================================================
  // FETCH PRINCIPALE (versione migliorata del tuo branch)
  // =========================================================
  useEffect(() => {
    ...
    async function loadAll() {
      try {
        // 1️⃣ Fetch dettagli (per mostrare subito contenuto)
        ...
        // 3️⃣ Rated TV e Film (in background)
        (async () => {
          try {
            const options = {
              method: "GET",
            ...
            const found =
              mediaType === "tv"
                ? rtRes.results?.find((x) => Number(x.id) === numericId)
                : rmRes.results?.find((x) => Number(x.id) === numericId);
  ...
  const scrollCast = (dir = 1) => {
    castRef.current?.scrollBy({
      left: dir * castRef.current.clientWidth,
      behavior: "smooth",
    });
  };
  ...
  const scrollSim = (dir = 1) => {
    similarRef.current?.scrollBy({
      left: dir * similarRef.current.clientWidth,
      behavior: "smooth",
    });
  };

  // =========================================================
  // Rating preferito (IT → fallback)
  // =========================================================
  const getPreferredRating = () => {
    if (!ratings) return null;

    if (mediaType === "tv") {
      const it = ratings.find((r) => r.iso_3166_1 === "IT");
      return it?.rating || ratings.find((r) => r.rating)?.rating || null;
    }

    // movie
    const it = ratings.find((r) => r.iso_3166_1 === "IT");
    if (it?.release_dates) {
      const cert = it.release_dates.find((d) => d.certification);
      if (cert) return cert.certification;
    }

    for (const row of ratings) {
      const cert = row.release_dates?.find((d) => d.certification);
      if (cert) return cert.certification;
    }

    return details?.adult ? "18+" : null;
  };
  ...
  const isFavourite =
    favourites?.some(
      (f) =>
        ...
        (f.media_type ||
          f.mediaType ||
          (f.first_air_date ? "tv" : "movie")) === mediaType
    ) || false;
      ...
      <div className="detail-header">
        ...
        <div className="detail-header-right">
          ...
          >
            {isFavourite ? (
              <svg width="22" height="22" viewBox="0 0 24 24" fill="white">
                <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5C2 5.42 4.42 3 7.5 3C9.24 3 10.91 3.81 12 5.09C13.09 3.81 14.76 3 16.5 3C19.58 3 22 5.42 22 8.5C22 12.28 18.6 15.36 13.35 20.04L12 21.35Z" />
              </svg>
            ) : (
              <svg
                width="22"
              ...
              >
                <path d="M12.1 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5C2 5.42 4.42 3 7.5 3C9.24 3 10.91 3.81 12 5.09C13.09 3.81 14.76 3 16.5 3C19.58 3 22 5.42 22 8.5C22 12.28 18.6 15.36 13.35 20.04L12 21.35Z" />
              </svg>
            )}
          </button>
          ...
          {typeof details.vote_average === "number" && (
            <div className="detail-aggregate-inline">
              <span className="agg-num">
